...
<p align="center">Mark web pages for use with large language models</p>
...
A `data-mark-id` attribute containing the label is also added to each marked element.
...
await page.locator('[data-mark-id="0"]').click();
...
// markedElements["0"].maskElement returns the bounding box element for the first marked element
```
